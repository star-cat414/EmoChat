...
import type { DistributionDatum } from "@/components/analytics/EmotionDistributionChart";
import type { TrendRow } from "@/components/analytics/EmotionTrendChart";
...
export function useEmotionAnalytics(query: {
  ...
  const load = useCallback(async () => {
    ...
    let builder = supabase
      ...
      .select("id, message_id, user_id, conversation_id, predicted_emotion, language, created_at")
  ...
  const distribution = useMemo<DistributionDatum[]>(() => {
    ...
    return EMOTIONS.map((e) => ({
      emotion: e,
      percent: Number((((counts[e as EmotionLabel] ?? 0) / total) * 100).toFixed(1)),
    }));
  }, [records]);
  ...
  const trend = useMemo<TrendRow[]>(() => {
    ...
    for (const r of records) {
      const key = r.created_at ? new Date(r.created_at).toISOString().slice(0, 10) : "";
      if (!key) continue;
      byDay[key] = byDay[key] ?? {};
      byDay[key][r.predicted_emotion] = (byDay[key][r.predicted_emotion] ?? 0) + 1;
    }
...
/** Filter trend rows to a time range. */
